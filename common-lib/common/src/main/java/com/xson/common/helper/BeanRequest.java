...
public class BeanRequest<T extends BaseBean> extends MultiPartRequest<T> {
    ...
    protected Response<T> parseNetworkResponse(NetworkResponse response) {
        ...
        try {
            ...
            if (baseBean != null && 1 == baseBean.getStatus()) {//状态混乱，出现问题不要惊讶（后台很神奇）
               switch (baseBean.getCode()){//错误代码（1已注册用户|11验证码发送成功|101验证超过上限|10参数缺失|100错误|404没有数据|0失败）
                   case 10:
                   case 100:
                   case 0:
                   case 101:
                       return Response.error(new UserLevelError(StringUtils.isEmpty(baseBean.getMsg())?baseBean.getInfo():baseBean.getMsg(), baseBean));
               }
            }else {
                return Response.error(new UserLevelError(StringUtils.isEmpty(baseBean.getMsg())?baseBean.getInfo():baseBean.getMsg(), baseBean));
            }
