...
import com.lanmei.lijia.WebSocket.UI.BaseFragment;
...
public abstract class AbsBaseWebSocketFragment extends BaseFragment {
    ...
    public void onEventMainThread(WebSocketSendDataErrorEvent event) {
        Log.d(TAG, String.format("onEventMainThread(WebSocketSendDataErrorEvent)->%s", event.getCause()));
        Log.d(TAG, String.format("onEventMainThread(WebSocketSendDataErrorEvent)->%s", event.getCause()));
